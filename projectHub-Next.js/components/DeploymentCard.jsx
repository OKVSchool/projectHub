...
export default function DeploymentCard({ endeavor }) {
  return (
    <Link href={`/endeavors/${endeavor._id}`}>
      <div style={{
        background: '#1a1a1a',
        ...
        <div style={{ padding: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 600 }}>{endeavor.title}</h2>
            <span style={{
              fontSize: '0.7rem',
              fontWeight: 600,
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {endeavor.repoUrl && (
              <a
                href={endeavor.repoUrl}
                target="_blank"
                rel="noopener noreferrer"
                onClick={e => e.stopPropagation()}
                style={{ fontSize: '0.75rem', color: '#e07820', textDecoration: 'none' }}
              >
                Repo ↗
              </a>
            )}
